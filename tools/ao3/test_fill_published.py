...
class FakeAO3:
    def get(self, url):
        ...
        if "/500" in url:
            return ("error", None)
        return ("ok", types.SimpleNamespace(text=work_page(), url=url, status_code=200))
...
requested.clear()
A.run_fill_published(FakeAO3(), csv_path=path, save_every=2)
check("resume refetches only 400 and 500", sorted(
    u.split("/works/")[1].split("?")[0] for u in requested), ["400", "500"])
